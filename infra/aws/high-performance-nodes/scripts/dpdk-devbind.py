...
def bind_device(pci_addr, driver):
    ...
    if driver == "vfio-pci":
        ...
        # Logic: Check if /sys/kernel/iommu_groups is empty. If it doesn't exist, we skip (matching original behavior).
        iommu_groups_count = -1
        try:
            iommu_groups_count = len(os.listdir("/sys/kernel/iommu_groups/"))
        except OSError:
            pass

        if iommu_groups_count == 0:
